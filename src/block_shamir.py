...
class BasicShamir:
    ...
    # ---------------------- 大密钥（最高 32KB）支持 ----------------------
    _LARGE_HEADER_LEN = 4  # bytes: [threshold(2)][block_count(2)]
    ...
    def split_secret_large(self, secret: bytes, n: int, t: int) -> List[Tuple[int, bytes]]:
        ...
        if len(secret) == 0:
            ...
            return [(i, header) for i in range(1, n + 1)]
        ...
        # 打包输出：每位参与者一个 bytes（带头部用于恢复阈值与块数）。
        return [(i + 1, header + bytes(buf)) for i, buf in enumerate(buffers)]

    def recover_secret_large(self, shares: List[Tuple[int, bytes]], t: Optional[int] = None) -> bytes:
        """
        从 *大密钥* 份额恢复原文。
        ...
        """
        if not shares:
            raise ValueError("Need at least one share to recover (large-secret)")
        ...
        for sid, payload in shares:
            if not isinstance(payload, (bytes, bytearray)):
                raise TypeError("large-secret shares must be bytes payloads")
            if len(payload) < header_len:
                raise ValueError("份额头部缺失，无法恢复（large-secret）")
            if sid not in uniq:
                uniq[sid] = bytes(payload)

        if not uniq:
            raise ValueError("需要至少一个合法份额（large-secret）")

        first_payload = next(iter(uniq.values()))
        expected_header = first_payload[:header_len]
        threshold_encoded = int.from_bytes(expected_header[:2], "big")
        block_count = int.from_bytes(expected_header[2:], "big")
        threshold = threshold_encoded if t is None else t
        if threshold < 2:
            raise ValueError("Need at least 2 shares for recovery (large-secret)")
        if len(uniq) < threshold:
            raise ValueError(f"Need at least {threshold} unique shares for recovery (large-secret)")

        # 校验所有份额的头部一致
        for payload in uniq.values():
            if payload[:header_len] != expected_header:
                raise ValueError("份额头部不一致，无法恢复（large-secret）")

        payload_length_expected = block_count * val_len
        for payload in uniq.values():
            body = payload[header_len:]
            if len(body) != payload_length_expected:
                raise ValueError("份额内容长度与头部信息不符（large-secret）")

        # 将选中的 t 份份额解包为 t 条 y 序列
        selected_items = sorted(uniq.items(), key=lambda x: x[0])[:threshold]
        share_ids = [sid for sid, _ in selected_items]
        payload_lists = []
        for _, packed in selected_items:
            body = packed[header_len:]
            ys = [
                int.from_bytes(body[i * val_len:(i + 1) * val_len], "big")
                for i in range(block_count)
            ]
            payload_lists.append(ys)

        out = bytearray()
        for block_index in range(block_count):
            shares_k = [
                (share_ids[idx], payload_lists[idx][block_index])
                for idx in range(len(share_ids))
            ]
    ...
    def _is_large_share_payload(self, value: object) -> bool:
        return isinstance(value, (bytes, bytearray)) and len(value) >= self._LARGE_HEADER_LEN
    ...
    def split_secret(self, secret: bytes, n: int, t: int) -> List[Tuple[int, Union[int, bytes]]]:
        """
        将秘密分割成份额列表。
        ...
        if len(secret) > self.block_size:
            return self.split_secret_large(secret, n, t)
    ...
    def recover_secret(self, shares: List[Tuple[int, Union[int, bytes]]]) -> bytes:
        """
        使用拉格朗日插值从份额恢复秘密。
        ...
        sample_value = shares[0][1]
        if self._is_large_share_payload(sample_value):
            return self.recover_secret_large(shares)
    ...
    def verify_shares_consistency(self, shares: List[Tuple[int, Union[int, bytes]]], t: int) -> bool:
        """
        验证份额集合是否来自同一多项式。
        ...
        sample_value = shares[0][1]
        if self._is_large_share_payload(sample_value):
            header = sample_value[:self._LARGE_HEADER_LEN]
            embedded_t = int.from_bytes(header[:2], "big")
            required_t = embedded_t if embedded_t >= 2 else 2

            uniq_subset: List[Tuple[int, bytes]] = []
            seen_ids = set()
            for sid, payload in shares:
                if sid in seen_ids:
                    continue
                if not self._is_large_share_payload(payload):
                    return False
                uniq_subset.append((sid, payload))
                seen_ids.add(sid)
                if len(uniq_subset) == required_t:
                    break

            if len(uniq_subset) < required_t:
                return False
            try:
                baseline = self.recover_secret_large(uniq_subset)
            except Exception:
                return False
            for subset in combinations(uniq_subset, required_t):
                try:
                    if self.recover_secret_large(list(subset)) != baseline:
                        return False
                except Exception:
                    return False
            return True
