...
class FeldmanVSS(BasicShamir):
    ...
    def batch_verification(self, shares: List[Tuple[int, int]], commitments: List[int]) -> List[bool]:
        """
        批量验证多份份额（基础版：无任何优化，仅保留最直接的验证逻辑）。
        核心：逐份验证，直接计算所有幂次和乘积，不做预处理或算法优化。
        """
        results = []
        ...
        p = self.p  # 大素数模
        q = self.q  # 有限域素数
        t = len(commitments)  # 承诺值数量（多项式阶数+1）
        ...
        for share_id, share_value in shares:
            ...
            # 计算验证等式右侧：∏(Cj^(i^j)) mod p
            rhs = 1
            for j in range(t):
                # 直接计算 i^j mod q（无迭代优化，每次调用pow）
                i_power = pow(share_id, j, q)
                # 直接计算 Cj^(i^j) mod p（无二进制分解，直接调用pow）
                cj_power = pow(commitments[j], i_power, p)
                # 累积乘积
                rhs = (rhs * cj_power) % p

            # 计算验证等式左侧：g^share_value mod p（直接调用pow）
